import { useApplyPromocode, useSendAllReportsToEmail } from '@/entities'
...
export const Footer = ({ packagesData }: Props) => {
  ...
  const { mutateAsync: sendAllReportsToEmailMutateAsync, isPending: sendAllReportsToEmailIsPending } =
    useSendAllReportsToEmail()
    ...
    return (
      <>
        <form className={css.form} onSubmit={handleSubmit(onSubmit)}>
          ...
          />
          <Button
            disabled={!isValid || sendAllReportsToEmailIsPending}
            type='submit'
            className={combaneCSS(css.button, css.sendToEmailButton)}
            variant='gradient'
          >
